...
def convert_image_to_grid_map(image_path):

    # This function takes in an image as an input and returns
    # 2 ox and oy lists of obstacles coordinates
    # to be used in D* Algorithm

    grid_map = cv2.imread(image_path, 0)
    map_reverted = cv2.bitwise_not(grid_map)
    new_map = map_reverted / 255.0
    ox_arr, oy_arr = np.where(new_map == 1)
...
class DStarLite:
    ...
    # possible motions that the algorithm can take
    motions = [
        Node(1, 0, 1),
        Node(0, 1, 1),
        Node(-1, 0, 1),
    ...
    def __init__(self, ox: list, oy: list):
        ...
        # End of world dimension computation
        ...
        self.start = Node(0, 0)  # Needs changing
        self.goal = Node(0, 0)  # Needs changing
...
def main():
    ...
    for image_name in image_paths:
        image_path = os.path.join("maps", image_name)
        ox, oy = convert_image_to_grid_map(image_path)
